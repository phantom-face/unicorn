...
import jp.face.phantom.unicorn.CursorAfterLastException;
 ...
 * @since DbUtils 1.1
 */
public class LimitedRecordCountBeanProcessor {

  /**
   * Special array value used by <code>mapColumnsToProperties</code> that indicates there is no bean
   * property that matches a column from a <code>ResultSet</code>.
   */
   ...
   */
  public LimitedRecordCountBeanProcessor() {
   ...
   * @since 1.5
   */
  public LimitedRecordCountBeanProcessor(Map<String, String> columnToPropertyOverrides) {
    super();
   ...
   * <ol>
   * <li>The class has a writable property with the same name as a column. The name comparison is
   * case insensitive.</li>
   *
   * <li>The column type can be converted to the property's set method parameter type with a
   * ResultSet.get* method. If the conversion fails (ie. the property was an int and the column was
   * a Timestamp) an SQLException is thrown.</li>
   * </ol>
   *
   * <p>
   * Primitive bean properties are set to their defaults when SQL NULL is returned from the
   * <code>ResultSet</code>. Numeric fields are set to 0 and booleans are set to false. Object bean
   * properties are set to <code>null</code> when SQL NULL is returned. This is the same behavior as
   ...
   * uses reflection and <code>BeanInfo</code> classes to match column names to bean property names.
   ...
   * @param rs ResultSet that supplies the bean data
   * @param type Class from which to create the bean instance
   * @param limit limit record size
   * @throws SQLException if a database access error occurs
   * @return the newly created List of beans
   */
  public <T> List<T> toBeanList(ResultSet rs, Class<T> type, int limit) throws SQLException {
    List<T> results = new ArrayList<T>();

    if (!rs.next()) {
      throw new CursorAfterLastException("cursor is after last.");
    }
   ...
   */
  private <T> T createBean(ResultSet rs, Class<T> type, PropertyDescriptor[] props,
  ...
  /**
   * Calls the setter method on the target object for the given property. If no setter method exists
   * for the property, this method does nothing.
   * 
   ...
   * property might take an Integer or a primitive int. This method returns true if the value can be
   * successfully passed into the setter method. Remember, Method.invoke() handles the unwrapping of
   * Integer into an int.
   *
   ...
   * <code>rs.getObject(index)</code> while more complex implementations could perform type
   * manipulation to match the column's type to the bean property type.
   *
   * <p>
   * This implementation calls the appropriate <code>ResultSet</code> getter method for the given
   * property type to perform the type conversion. If the property type doesn't match one of the
   * supported <code>ResultSet</code> types, <code>getObject</code> is called.
   * </p>
   *
   ...
   * @throws SQLException if a database access error occurs
   *
   * @return The object from the <code>ResultSet</code> at the given column index after optional
   *         type processing or <code>null</code> if the column value was SQL NULL.
   */
